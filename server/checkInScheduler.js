...
const SCHEDULER_INTERVAL_MS = 60 * 1000; // run every 60 seconds
  ...
  try {
    ...
    for (const user of dueUsers) {
      try {
        ...
        // Avoid creating duplicate pending sessions
        ...
        if (existingPending) {
          console.log('[CheckInScheduler] User', user._id.toString(), 'already has pending session', existingPending._id.toString());
          continue;
        }
